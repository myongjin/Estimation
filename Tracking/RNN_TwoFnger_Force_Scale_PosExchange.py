...
# 무조건 첫번째 데이터 오차가 더 작게 나오는 것인지 확인하기 위함
...
def build_dataset(time_series, seq_length):
    ...
    for i in range(0, len(time_series) - seq_length):
        ...
        x[:, 0:3] = time_series[i:i + seq_length, 9:12]
        x[:, 3:6] = time_series[i:i + seq_length, 1:4]
        x[:, 6:] = time_series[i:i + seq_length, 17:17 + 2288]

        y = np.zeros(2)
        y[0] = time_series[i + seq_length, 16]
        y[1] = time_series[i + seq_length, 8]
...
load_model_name = 'Models/Twohands_force_1hidden_seq3_exchangeTest_16082021.h5'

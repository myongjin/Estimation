...
for id in idList:
    loadname = 'ID' + id + '_filtered_MAF_pos3'
    print(loadname)
    ...
    if do_analysis:
        for _startIdx in range(0, l_min, l_interval):
            for l_Sequence in range(l_min, l_max + l_interval, l_interval):
                ...
                if drawGeneratedSequence and not analyseAllData:
    ...
    ########################################### load analysed data ##########################################
    if not do_analysis and not analyseAllData:
    ...
    # draw patterns on 3D
    if drawBestMatch and len(matchedArray) and not analyseAllData > 0:
